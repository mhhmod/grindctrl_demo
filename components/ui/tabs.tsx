...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        /* text-foreground/60 measured at 4.13:1 against a light tab-list
           background — under WCAG AA's 4.5:1 floor for normal text (caught
           by e2e/golden-standard-a11y.spec.ts). /70 clears it with margin
           (~5.6:1) and still reads as visually secondary next to the active
           tab's full-strength text-foreground. Dark mode's token-based
           dark:text-muted-foreground already clears AA (~6.6:1+) and is
           untouched. */
        "relative inline-flex h-[calc(100%-1px)] flex-1 items-center justify-center gap-1.5 rounded-xl border border-transparent px-2 py-1 text-sm font-medium whitespace-nowrap text-foreground/70 transition-all group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start group-data-vertical/tabs:px-2.5 group-data-vertical/tabs:py-1.5 hover:text-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-1 focus-visible:outline-ring disabled:pointer-events-none disabled:opacity-50 has-data-[icon=inline-end]:pe-1.5 has-data-[icon=inline-start]:ps-1.5 dark:text-muted-foreground dark:hover:text-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
